...
const pathBoard = computed(()=>{return {
  tipo: 'Subpagina',
  index: paginaStore.pagina[$cms('container')].findIndex(p => p[$cms('id')] === props.realSubpagina[$cms('id')]),
...
function selecionarContainer(board){
  if(board.subpaginaAtivaId){
    ferramentaStore.selecionarSubpagina({boardId: board.id, id: board.subpaginaAtivaId},{
      tipo: 'subpagina',
      index: usePaginaStore().pagina[$cms('container')].findIndex(p => p[$cms('id')] === board.subpaginaAtivaId),
